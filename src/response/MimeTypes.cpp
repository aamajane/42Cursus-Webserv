...
MimeTypes::MimeTypes() : mimeTypes()
{
	...
	this->mimeTypes["py"] = "application/octet-stream";
...
{
	if (this->mimeTypes.count(exe))
		return this->mimeTypes[exe];
	return "";
}

const std::string	MimeTypes::getExeFile(const std::string& ContentType)
{
	for (std::map<std::string, std::string>::iterator it = this->mimeTypes.begin(); it != this->mimeTypes.end(); ++it)
	{
